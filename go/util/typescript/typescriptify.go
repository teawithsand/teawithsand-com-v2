...
import (
	...
	// TODO(teawithsand): get rid of this reflector stuff
...
func (ts Converter) logf(depth int, s string, args ...interface{}) {
...
func (t *Converter) ManageType(fld interface{}, opts TypeOptions) *Converter {
	var typ reflect.Type
	switch t := fld.(type) {
	case reflect.Type:
		typ = t
...
func (t *Converter) WithCreateFromMethod(b bool) *Converter {
	t.CreateFromMethod = b
	return t
}

func (t *Converter) WithInterface(b bool) *Converter {
	t.CreateInterface = b
	return t
}

func (t *Converter) WithConstructor(b bool) *Converter {
	t.CreateConstructor = b
	return t
}

func (t *Converter) WithIndent(i string) *Converter {
	t.Indent = i
	return t
}

func (t *Converter) WithBackupDir(b string) *Converter {
	t.BackupDir = b
	return t
}

func (t *Converter) WithPrefix(p string) *Converter {
	t.Prefix = p
	return t
}

func (t *Converter) WithSuffix(s string) *Converter {
	t.Suffix = s
	return t
}

func (t *Converter) AddVariable(name string, value interface{}) *Converter {
	if t.variables == nil {
...
func (t *Converter) Add(obj interface{}) *Converter {
	switch ty := obj.(type) {
	case StructType:
...
func (t *Converter) AddType(typeOf reflect.Type) *Converter {
...
func (t *Converter) AddEnum(values interface{}) *Converter {
	if t.enums == nil {
...
func (t *Converter) AddEnumValues(typeOf reflect.Type, values interface{}) *Converter {
	t.AddEnum(values)
	return t
}

func (t *Converter) Convert(customCode map[string]string) (string, error) {
	...
	for k, v := range t.variables {
		...
		default:
			...
			res, err := json.Marshal(tv)
			if err != nil {
				return "", err
			}

			text, err := json.Marshal(string(res))
			if err != nil {
				return "", err
			}

			result += fmt.Sprintf(`%sexport const %s = JSON.parse(%s);`, "\n", k, string(text))
		}

		if !manual {
			result += fmt.Sprintf(`%sexport const %s = %s;`, "\n", k, v)
		}
	}
...
func (t Converter) backup(fileName string) error {
...
func (t Converter) ConvertToFile(fileName string) error {
...
func (t *Converter) convertEnum(depth int, typeOf reflect.Type, elements []enumElement) (string, error) {
...
func (t *Converter) getFieldOptions(structType reflect.Type, field reflect.StructField) TypeOptions {
...
func (t *Converter) getJSONFieldName(field reflect.StructField, isPtr bool) string {
	jsonFieldName := ""
...
func (t *Converter) convertType(depth int, typeOf reflect.Type, customCode map[string]string) (string, error) {
	if _, found := t.alreadyConverted[typeOf]; found { // Already converted
		return "", nil
	}
...
func (t *Converter) AddImport(i string) {
	for _, cimport := range t.customImports {
		if cimport == i {
			return
		}
