...
import { RemoteWordSetGetPublicListRequest, RemoteWordSetPublicSummaryProjection } from "@app/exported/typings/api"
...
const bemParent = simpleBemParent("page-langka-word-set-public-list")
...
export default () => {
    ...
    const [state, info, ops] = useRemoteDataStream({
        stateFactory: (): {
            isAfterLast: boolean,
            entries: RemoteWordSetPublicSummaryProjection[],
        } => ({
            isAfterLast: true,
            entries: [],
        }),
        ...
        loader: async (page: number): Promise<RemoteWordSetPublicSummaryProjection[]> => {
            const res = await wordSetClient.getPublicWordSetList({
                ...typedParameters,
                offset: page * perPage,
                limit: perPage
            })
    ...
    if (info.type === "error") {
        ...
        footerContent = <FormattedMessage id="langka.page.wordset.list_public.no_more_entries" />
    } else {
        footerContent = <button className="btn btn-primary" onClick={() => ops.enqueueLoad(page + 1)}>
            <FormattedMessage id="langka.page.wordset.list_public.load_more" />
        </button>
    }
    ...
    return <div className={bemRoot()}>
        <header className={bemHeader()}>
            <h1>
                <FormattedMessage id="langka.page.wordset.list_public.title" />
            </h1>
        </header>
        ...
        <main className={bemEntries()}>
            {
                state.entries.length > 0 ?
                    state.entries.map((e) => {
                        return <WordSetCard data={e} type="public" key={e.id}
                            showShowButton={true}
                    ...
                    :
                    <div>
                        <FormattedMessage id="langka.page.wordset.list_public.no_entries" />
                    </div>
            }
        </main>
